        ...
        for(let i=0; i<moves.length; i++) {
            ...
            for(let j=0; j<next.length; j++) {
                ...
                if(score[1] < best_blunderscore) {
                    chosen_move = move;
                    chosen_world = (move.get_name() == Rules.Attacker_Might_Sphere_Selection
                                || move.get_name() == Rules.Attacker_Would_Sphere_Selection) ? next[j].get_delim_world().index : next[j].get_current_world().index;
